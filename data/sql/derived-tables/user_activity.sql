...
CREATE MATERIALIZED VIEW user_activity AS (
	...
	CASE WHEN u.subscribed_member IS FALSE
	    THEN greatest(
	    	gambit_unsub.sms_unsubscribed_at,
    ...
    LEFT JOIN (
    	SELECT DISTINCT
		id,
		FIRST_VALUE(updated_at) OVER (PARTITION BY id
		ORDER BY updated_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS unsub_ts
	...
	FROM (
	    SELECT DISTINCT
		customer_id,
		LAST_VALUE("timestamp") OVER (PARTITION BY customer_id ORDER BY "timestamp", event_type
		    RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_ts,
		LAST_VALUE(event_type) OVER (PARTITION BY customer_id ORDER BY "timestamp", event_type
	...
	FROM (
		...
		LAST_VALUE(mel.next_action_ts - r.post_created_at) OVER (
		    ...
		    ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS time_to_next_action_last_rb
	    FROM reportbacks r
	    LEFT JOIN LATERAL (
...
GRANT SELECT ON public.user_activity TO looker;
GRANT SELECT ON public.user_activity TO dsanalyst;
