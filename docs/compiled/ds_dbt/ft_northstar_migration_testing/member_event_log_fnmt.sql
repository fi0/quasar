...
SELECT DISTINCT 
    ...
    'web' AS channel
FROM "quasar_prod_warehouse"."public"."northstar_users_deduped_fnmt" u_access
...
SELECT DISTINCT 
    ...
    '0' AS action_serial_id,
    'web' AS channel
FROM "quasar_prod_warehouse"."public"."northstar_users_deduped_fnmt" u_login
...
FROM
    ...
    FROM "quasar_prod_warehouse"."public"."northstar_users_deduped_fnmt" u_create
...
INNER JOIN "quasar_prod_warehouse"."public"."users_fnmt" u
ON b.northstar_id = u.northstar_id
WHERE b.northstar_id IS NOT NULL
AND b.interaction_type IS DISTINCT FROM 'preview'
) AS a
LEFT JOIN "quasar_prod_warehouse"."public"."users_fnmt" u ON u.northstar_id = a.northstar_id