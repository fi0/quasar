...
rb_summary AS (
	...
	FROM "postgres"."rpacas"."reportbacks"
    ) r_with_lag
...
gambit_unsub AS (
	...
	FROM "postgres"."rpacas"."gambit_messages_inbound"
    ) f
),
...
LEFT JOIN (
    ...
    FROM "postgres"."rpacas_ft_dosomething_rogue_qa"."posts"
) p
