...
rb_summary AS (
	...
	FROM "quasar_prod_warehouse"."dbt_sena"."reportbacks"
    ) r_with_lag
...
gambit_unsub AS (
	...
	FROM "quasar_prod_warehouse"."dbt_sena"."gambit_messages_inbound"
    ) f
),
...
time_to_actions AS (
	...
	FROM "quasar_prod_warehouse"."dbt_sena"."reportbacks" r
	LEFT JOIN LATERAL (
	    SELECT "timestamp" AS next_action_ts, action_type AS next_action_type
	    FROM "quasar_prod_warehouse"."dbt_sena"."mel"
...
FROM "quasar_prod_warehouse"."dbt_sena"."users" u
LEFT JOIN (
SELECT
    northstar_id,
    count(DISTINCT campaign_id) AS num_signups,
    max(created_at) AS most_recent_signup
FROM "quasar_prod_warehouse"."dbt_sena"."signups"
